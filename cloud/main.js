...
function updateUtopianPosts(perm, auth, lastPermlink) {
    ...
    steem.api.getDiscussionsByCreated(query, function(err, results) {
        ...
        for (result of results) {
            ...
            if (!done && result.active_votes.find(function(e) {
                    return e.voter == "utopian-io"
                }) === undefined && (JSON.parse(result.json_metadata).moderator === undefined || !JSON.parse(result.json_metadata).moderator.flagged) && result.beneficiaries.find(function(e) {
                    return e.account = "utopian.pay";
                }) !== undefined) {
                    ...
                    success: function(posts) {
                        if (posts !== undefined && posts.length !== 0) {
                            ...
                            steem.broadcast.vote(WIF, BOT, post.get('author'), post.get('permlink'), 10000, function(err, result) {
                                ...
                                steem.broadcast.comment(WIF, post.get('author'), post.get('permlink'), BOT, permlink, "", body, {
                                    "app": "1up"
                                }, function(err, result) {
                                    console.log(err, result);
